...
internal struct UserAuthSignablePayload {
    ...
    init(sessionIdentifier: ByteBuffer, userName: String, serviceName: String, publicKey: NIOSSHPublicKey) {
        ...
        // [Berth patch] Sign over the advertised signature algorithm (rsa-sha2-512 for
        // RSA) so the signed blob matches what the server expects per RFC 8332 §3.3.
        newBuffer.writeSSHString(publicKey.userAuthPrefix)
