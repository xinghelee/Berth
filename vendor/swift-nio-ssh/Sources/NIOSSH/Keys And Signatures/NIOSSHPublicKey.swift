...
extension NIOSSHPublicKey {
    ...
    /// [Berth patch] The signature-algorithm name to advertise in user auth (RFC 8332).
    /// Equals `keyPrefix` for every built-in key; only a custom key (Citadel RSA) may
    /// differ, letting the key blob stay `ssh-rsa` while auth negotiates `rsa-sha2-512`.
    var userAuthPrefix: String.UTF8View {
        switch self.backingKey {
        case .custom(let publicKey):
            return publicKey.userAuthPrefix.utf8
        default:
