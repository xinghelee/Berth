...
public protocol NIOSSHPublicKeyProtocol {
    ...
    /// [Berth patch] The signature algorithm name advertised during user authentication
    /// (RFC 8332). Defaults to `publicKeyPrefix`. RSA overrides this to `rsa-sha2-512`
    /// so the *key blob* stays `ssh-rsa` while the *signature algorithm* is SHA-2, which
